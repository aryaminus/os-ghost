...
use crate::input::{InputController, MouseButton, Key, ScrollDirection};
...
impl McpTool for ClickTool {
    fn descriptor(&self) -> ToolDescriptor {
        ...
        properties.insert("x".to_string(), PropertySchema {
            prop_type: "integer".to_string(),
            description: Some("X coordinate".to_string()),
            default: None,
            enum_values: None,
        });
        properties.insert("y".to_string(), PropertySchema {
            prop_type: "integer".to_string(),
            description: Some("Y coordinate".to_string()),
            default: None,
            enum_values: None,
        });
        properties.insert("button".to_string(), PropertySchema {
    ...
    async fn execute(&self, arguments: serde_json::Value) -> Result<serde_json::Value, McpError> {
        let x = arguments["x"].as_i64().ok_or_else(|| McpError::InvalidArguments("Missing x".into()))? as i32;
        let y = arguments["y"].as_i64().ok_or_else(|| McpError::InvalidArguments("Missing y".into()))? as i32;
        ...
        self.input_controller.move_mouse(x, y).await
            .map_err(|e| McpError::ExecutionFailed(format!("Mouse move failed: {}", e)))?;
        
        self.input_controller.click_mouse(button).await
...
impl McpTool for TypeTool {
    fn descriptor(&self) -> ToolDescriptor {
        let mut properties = HashMap::new();
        properties.insert("text".to_string(), PropertySchema {
            prop_type: "string".to_string(),
    ...
    async fn execute(&self, arguments: serde_json::Value) -> Result<serde_json::Value, McpError> {
        let text = arguments["text"].as_str().ok_or_else(|| McpError::InvalidArguments("Missing text".into()))?;
        
        info!("Typing text ({} chars)", text.len());
        
        self.input_controller.type_text(text).await
...
impl McpTool for ScrollTool {
    fn descriptor(&self) -> ToolDescriptor {
        let mut properties = HashMap::new();
        properties.insert("direction".to_string(), PropertySchema {
            prop_type: "string".to_string(),
        ...
        properties.insert("amount".to_string(), PropertySchema {
            prop_type: "integer".to_string(),
    ...
    async fn execute(&self, arguments: serde_json::Value) -> Result<serde_json::Value, McpError> {
        let direction_str = arguments["direction"].as_str()
        ...
        self.input_controller.scroll(direction, amount).await
...
impl McpTool for KeyTool {
    fn descriptor(&self) -> ToolDescriptor {
        let mut properties = HashMap::new();
        properties.insert("key".to_string(), PropertySchema {
            prop_type: "string".to_string(),
    ...
    async fn execute(&self, arguments: serde_json::Value) -> Result<serde_json::Value, McpError> {
        let _key = arguments["key"].as_str().ok_or_else(|| McpError::InvalidArguments("Missing key".into()))?;
        
        info!("Pressing key");
        
        // Simplified - just press space for now
        self.input_controller.press_key(Key::Space).await
