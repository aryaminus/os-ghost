...
impl From<&crate::integrations::integrations::Note> for NoteSnapshot {
    fn from(note: &crate::integrations::integrations::Note) -> Self {
        Self {
            id: note.id.clone(),
...
impl From<NoteSnapshot> for crate::integrations::integrations::Note {
        ...
        ActionState::NoteChange { before, after, .. } => {
            ...
            match (before, after) {
                (Some(snapshot), Some(_)) => {
                    let note: crate::integrations::integrations::Note = snapshot.clone().into();
                    store
