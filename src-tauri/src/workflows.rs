...
pub fn plan_for_intent(action_type: &str, args: &serde_json::Value) -> WorkflowPlan {
    let id = format!("workflow_{}", crate::utils::current_timestamp());
    let title = format!("Workflow for {}", action_type);
    let step = WorkflowStep {
        id: format!("step_{}", crate::utils::current_timestamp()),
    ...
    let verify = WorkflowStep {
        id: format!("step_verify_{}", crate::utils::current_timestamp()),
        ...
        "verify.notes" => {
            ...
            match action_type {
                "notes.add" | "notes.update" => {
                    ...
                    let note: Option<crate::integrations::Note> = store
                        .get("notes", id)
