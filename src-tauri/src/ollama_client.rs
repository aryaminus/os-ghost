...
use crate::utils::{clean_json_response, runtime_config};
...
impl OllamaClient {
    ...
    pub async fn generate_dynamic_puzzle(
        ...
        page_content: &str,
        history_context: &str,
    ) -> Result<crate::gemini_client::DynamicPuzzle> {
        tracing::info!(
...
Respond ONLY with valid JSON, no markdown."#,
        ...
        let puzzle: crate::gemini_client::DynamicPuzzle = serde_json::from_str(&text)
    ...
    ) -> Result<crate::gemini_client::VerificationResult> {
        let prompt = format!(
...
Be strict and accurate. Only return true if the visual proof CLEARLY matches the specific target description."#,
        ...
        let result: crate::gemini_client::VerificationResult = serde_json::from_str(clean_text)?;
        Ok(result)
    }
    ...
    pub async fn generate_adaptive_puzzle(
        &self,
        activities: &[crate::gemini_client::ActivityContext],
        current_app: Option<&str>,
        _current_content: Option<&str>,
    ) -> Result<crate::gemini_client::AdaptivePuzzle> {
...
Respond ONLY with valid JSON."#,
        ...
        let puzzle: crate::gemini_client::AdaptivePuzzle = serde_json::from_str(&text)
...
mod tests {
    ...
    fn test_runtime_config_defaults() {
        ...
        assert_eq!(
            ...
            crate::utils::DEFAULT_OLLAMA_URL
        );
        assert_eq!(
            config.get_ollama_vision_model(),
            crate::utils::DEFAULT_OLLAMA_VISION_MODEL
        );
        assert_eq!(
            config.get_ollama_text_model(),
            crate::utils::DEFAULT_OLLAMA_TEXT_MODEL
        );
    }
}
