use crate::integrations::{EmailMessage, EmailSettings, EmailTriageDecision};
...
fn update_sync_timestamp() {
    ...
    settings.last_sync_at = Some(crate::utils::current_timestamp());
    let _ = settings.save();
}
...
pub async fn triage_inbox(limit: usize, ai_router: Option<&crate::ai_provider::SmartAiRouter>) -> Result<Vec<EmailTriageDecision>> {
    ...
    for message in &messages {
        ...
        decisions.push(EmailTriageDecision {
...
pub async fn apply_triage(decisions: &[EmailTriageDecision]) -> Result<()> {
    let access = get_access_token().await?;
