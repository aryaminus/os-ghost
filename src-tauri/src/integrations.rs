...
pub fn email_begin_oauth(provider: String) -> Result<EmailSettings, String> {
    ...
    tauri::async_runtime::block_on(async {
        crate::email::begin_oauth().await.map_err(|e| e.to_string())
    })
}
...
pub fn email_disconnect() -> Result<EmailSettings, String> {
    tauri::async_runtime::block_on(async {
        crate::email::disconnect().await.map_err(|e| e.to_string())
    })
}
...
pub fn list_email_inbox(limit: Option<usize>) -> Vec<EmailMessage> {
    ...
    tauri::async_runtime::block_on(async {
        match crate::email::list_inbox(count).await {
            Ok(items) => items,
            Err(_) => Vec::new(),
        }
    })
...
pub fn triage_email_inbox(
    limit: Option<usize>,
    ai_router: State<'_, Arc<crate::ai_provider::SmartAiRouter>>,
    ...
    let count = limit.unwrap_or(settings.inbox_limit).clamp(1, 50);
    tauri::async_runtime::block_on(async {
        match crate::email::triage_inbox(count, Some(ai_router.as_ref())).await {
            Ok(items) => items,
            Err(_) => Vec::new(),
        }
    })
...
pub fn apply_email_triage(decisions: Vec<EmailTriageDecision>) -> Result<(), String> {
    tauri::async_runtime::block_on(async {
        crate::email::apply_triage(&decisions)
            .await
...
pub fn add_note(
    ...
    if let Some(rollback) = crate::rollback::get_rollback_manager() {
        rollback.record_note_change(
            ...
            &note.id,
            None,
            Some(crate::rollback::NoteSnapshot::from(&note)),
...
pub fn update_note(
    ...
    let before = notes_store
        .get_note(&note.id)
        .map_err(|e| e.to_string())?
        .map(|n| crate::rollback::NoteSnapshot::from(&n));
    ...
    if let Some(rollback) = crate::rollback::get_rollback_manager() {
        rollback.record_note_change(
            ...
            &updated.id,
            before,
            Some(crate::rollback::NoteSnapshot::from(&updated)),
