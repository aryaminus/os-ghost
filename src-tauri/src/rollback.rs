...
impl UndoStack {
    ...
    pub fn set_undo_executor<F>(&mut self, executor: F)
    where
        F: Fn(&UndoableAction) -> Result<(), String> + Send + Sync + 'static,
    {
        self.undo_executor = Some(Box::new(executor));
    }
    ...
    pub fn undo(&self) -> UndoResult {
        ...
        match action {
            Some(mut action) => {
                ...
                if let Some(executor) = &self.undo_executor {
...
impl RollbackManager {
    ...
    pub fn record_effect(&self, action_id: &str, effect_name: &str, duration_ms: u64) {
        self.undo_stack.record_effect(action_id, effect_name, duration_ms);
    }
