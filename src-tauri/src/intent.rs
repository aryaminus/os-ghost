...
use crate::events_bus::{EventEntry, EventKind, EventPriority};
...
pub fn get_intents(limit: Option<usize>) -> Vec<IntentCandidate> {
    let events = crate::events_bus::list_recent_events(limit.unwrap_or(40), 0);
...
pub fn auto_create_top_intent(
    ...
    let persona = crate::persona::get_persona();
    ...
    if !crate::intent_autorun::should_auto_create_intent(
        autonomy,
        last_intent_at,
        cooldown_override,
    ) {
    ...
    let events = crate::events_bus::list_recent_events(40, 0);
