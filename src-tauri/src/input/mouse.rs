...
use super::{MouseButton, ScrollDirection, InputError};
...
pub mod macos {
    ...
    pub fn move_mouse(_x: i32, _y: i32) -> Result<(), InputError> {
        ...
        Err(InputError::PlatformError("macOS mouse control requires accessibility permissions".to_string()))
    }
        ...
        unsafe {
            let wheel_delta = match direction {
                ScrollDirection::Up | ScrollDirection::Down => {
                    let delta = if matches!(direction, ScrollDirection::Up) { amount } else { -amount };
