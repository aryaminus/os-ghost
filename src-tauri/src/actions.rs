...
use std::sync::RwLock;
use crate::action_ledger::{update_action_status, ActionLedgerStatus};
...
pub struct HandlerContext {
    ...
    pub effect_queue: std::sync::Arc<crate::game_state::EffectQueue>,
    ...
    pub notes_store: std::sync::Arc<crate::integrations::NotesStore>,
}
...
fn handle_browser_navigate(
    ...
    Box::pin(async move {
        let url = ctx
            ...
            .and_then(|v| v.as_str())
            .ok_or("Missing URL")?;
        ctx.effect_queue.push(crate::game_state::EffectMessage {
...
fn handle_sandbox_write_file(
    ...
    Box::pin(async move {
        ...
        let plan = crate::workflows::plan_for_action("sandbox.write_file", &ctx.args);
        let sim = crate::workflows::simulate_plan(&plan);
        if !sim.success {
            return Err(format!(
                "guardrail: {}",
        ...
        let step_result = crate::workflows::execute_plan(&plan, |step| {
                ...
                match step_action_type.as_str() {
                    "sandbox.write_file" => {
                        ...
                        if result.success {
                            ...
                            Ok(crate::workflows::WorkflowStepResult {
                                output_text: None,
                    ...
                    _ => Ok(crate::workflows::WorkflowStepResult {
                        output_text: None,
                        output_json: None,
                        action_id: None,
...
fn handle_sandbox_list_dir(
    ...
    Box::pin(async move {
        let action_id = ctx.action_id;
        let plan = crate::workflows::plan_for_action("sandbox.list_dir", &ctx.args);
        let sim = crate::workflows::simulate_plan(&plan);
        if !sim.success {
            return Err(format!(
                "guardrail: {}",
        ...
        let workflow_result = crate::workflows::execute_plan(&plan, |step| {
            let args = step.arguments.clone();
            let step_action_type = step.action_type.clone();
            async move {
                match step_action_type.as_str() {
...
fn handle_sandbox_shell(
    ...
    Box::pin(async move {
        let action_id = ctx.action_id;
        let plan = crate::workflows::plan_for_action("sandbox.shell", &ctx.args);
        let sim = crate::workflows::simulate_plan(&plan);
        if !sim.success {
            return Err(format!(
                "guardrail: {}",
...
fn handle_notes_update(
    ...
    Box::pin(async move {
        let note = ctx
            ...
            .cloned()
            .ok_or("Missing note")?;
        let parsed: crate::integrations::Note =
...
fn handle_intent_summarize_page(
    ...
    Box::pin(async move {
        ...
        let plan = crate::workflows::plan_for_intent("intent.summarize_page", &ctx.args);
        let sim = crate::workflows::simulate_plan(&plan);
        if !sim.success {
        ...
        let redacted = crate::privacy::redact_with_settings(&content);
        ...
        let workflow_result = crate::workflows::execute_plan(&plan, |_step| {
...
fn handle_intent_create_tasks(
    ...
    Box::pin(async move {
        ...
        let plan = crate::workflows::plan_for_intent("intent.create_tasks", &ctx.args);
        let sim = crate::workflows::simulate_plan(&plan);
        if !sim.success {
            return Err(format!("guardrail: {}", sim.error.unwrap_or_else(|| "Workflow simulation failed".to_string())));
        }
...
fn handle_intent_draft_reply(
    ...
    Box::pin(async move {
        ...
        let plan = crate::workflows::plan_for_intent("intent.draft_reply", &ctx.args);
        let sim = crate::workflows::simulate_plan(&plan);
        if !sim.success {
            return Err(format!("guardrail: {}", sim.error.unwrap_or_else(|| "Workflow simulation failed".to_string())));
        }
...
pub async fn execute_approved_action(
    ...
    notes_store: State<'_, std::sync::Arc<crate::integrations::NotesStore>>,
    ...
    crate::events_bus::record_event(
        ...
        action.reason.clone(),
        std::collections::HashMap::new(),
        crate::events_bus::EventPriority::Low,
...
fn record_workflow_rollback(action_id: u64, action_type: &str, reason: &str) {
    ...
    let rollback = crate::workflows::rollback_action(action_type, action_id);
    if !rollback.success {
        crate::events_bus::record_event(
            crate::events_bus::EventKind::Guardrail,
...
fn maybe_create_skill(action: &PendingAction) {
    ...
    if crate::skills::increment_usage_for(&action.action_type, &trigger) {
        return;
    }
    if crate::skills::has_skill(&action.action_type, &trigger) {
        return;
    }
    ...
    let _ = crate::skills::create_skill_internal(
        title,
        description,
        trigger,
