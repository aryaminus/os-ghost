...
const Ghost = () => {
	...
	const extensionConnectedValue =
		extensionConnected || systemStatus?.extensionConnected;
	const extensionHealthy =
		systemStatus?.extensionOperational ?? extensionConnectedValue;
	...
	const sourceLabel = useMemo(() => {
		...
		if (
			(hasRecentScreenshot || systemSettings?.monitor_enabled) &&
			hasConsent
		) {
	...
	const lastScanLabel = useMemo(() => {
		...
		const delta = Math.max(
			0,
			Math.floor(Date.now() / 1000 - lastScreenshotAt),
		);
	...
	const displayText = useMemo(() => {
		...
		if (
			gameState.dialogue &&
			!gameState.dialogue.includes("Waiting for signal")
		) {
	...
	const shouldSkipTypewriter = useMemo(() => {
		...
		return (
			systemMessages.some((msg) => displayText.startsWith(msg)) ||
	...
	useEffect(() => {
		...
		const loadTimeline = async () => {
			try {
				const entries = await invoke("get_timeline", {
					limit: 6,
					offset: 0,
				});
	...
	const handleToggleReadOnly = useCallback(async () => {
		...
		const updated = await safeInvoke(
			"update_privacy_settings",
			{
				...
				browserTabCaptureConsent:
					privacySettings.browser_tab_capture_consent,
	...
	const systemState = useMemo(() => {
		...
		if (!extensionConnectedValue)
			return { status: "warning", label: "No extension" };
		if (!extensionHealthy)
			return { status: "warning", label: "Extension stale" };
	...
	return (
		...
		>
			...
			<header className="ghost-header">
				...
				{extensionConnectedValue &&
					!systemStatus?.extensionProtocolVersion &&
					!systemStatus?.lastExtensionHello && (
						<button
							type="button"
						...
						>
							...
							<span>
								Extension handshake missing. Reload extension.
							</span>
						</button>
					)}
				{extensionConnectedValue &&
					systemStatus?.extensionProtocolVersion === "legacy" && (
						<button
							type="button"
						...
						>
							...
							<span>
								Legacy extension detected. Update required.
							</span>
						</button>
					)}
				{latestNotification && (
					...
					{gameState.proximity > 0 && (
						<div className="proximity-bar">
							<div
								...
								style={{
									width: `${Math.min(100, gameState.proximity * 100)}%`,
								}}
							/>
						</div>
					)}
					...
					<div className="dialogue-scroll">
						...
						{companionBehavior?.trigger_context && (
							<div className="companion-line subtle">
								Why: {companionBehavior.trigger_context}
							</div>
						)}
						...
						{quickAsk.isOpen && (
							<form
								className="quick-ask-row"
								onSubmit={handleQuickAskSubmit}
							>
								<div className="quick-ask-field">
									<label
										className="quick-ask-label"
										htmlFor="quick-ask-input"
									>
										...
										{actionLedger.length > 0 && (
											<div className="timeline-section">
												...
												{actionLedger
													...
													.map((entry) => (
														<div
															key={`${entry.action_id}-${entry.timestamp}`}
			...
			{pendingActions.length > 0 && (
				...
				>
					...
					<div className="pending-desc">
						...
						{pendingActions[0].action_type?.startsWith(
							"intent.",
			...
			{actionPreview && (
				...
				>
					<div className="pending-title">
						Preview: {actionPreview.action?.action_type}
					</div>
					<div className="pending-desc">
						{actionPreview.action?.description}
						{actionPreview.action?.action_type?.startsWith(
							"intent.",
						) && <span className="pending-tag intent">Intent</span>}
						{actionPreview.action?.action_type?.startsWith(
							"sandbox.",
					...
					{actionPreview.action?.target && (
						<div className="pending-detail">
							Target: {actionPreview.action.target}
						</div>
					)}
					{actionPreview.action?.reason && (
						<div className="pending-why">
							Why: {actionPreview.action.reason}
						</div>
					)}
					{actionPreview.action?.risk_level && (
						<div className="pending-risk">
							Risk: {actionPreview.action.risk_level}
						</div>
					)}
					...
					{actionPreview.visual_preview && (
						<div className="preview-visual">
							{actionPreview.visual_preview.preview_type ===
								"screenshot" && (
								<img
									src={
										actionPreview.visual_preview.content.startsWith(
											"data:",
										)
											? actionPreview.visual_preview
													.content
									...
									alt={
										actionPreview.visual_preview.alt_text ||
										"Preview"
									}
								/>
							)}
					...
					{/* Editable params */}
					{actionPreview.editable_params &&
						Object.keys(actionPreview.editable_params).length >
							0 && (
					...
					{actionPreview.requires_approval && (
						<div className="pending-why">
							Approval required:{" "}
							{actionPreview.approval_summary ||
